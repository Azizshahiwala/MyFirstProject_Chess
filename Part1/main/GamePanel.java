    ...
    private void update()
    {
        StalemateCondition = isStalemate();
            ...
            //If there's no active piece go here
                ...
                int clickRow = mouse.x / Board.Full_block;
                int clickCol = mouse.y / Board.Full_block;
...
return false;
    ...
    if(ActivePiece.CanMove(ActivePiece.row,ActivePiece.col)){
            ...
            // Only remove captured piece if move is valid
            if(ActivePiece.PieceHit != null) {
                ActivePiece.PieceHit.captured = true;  // Mark as captured
   ...
   {    if(ActivePiece.Ids == Piece_types.PAWN){
    ...
    //Core game update loop - handles piece selection and move execution
